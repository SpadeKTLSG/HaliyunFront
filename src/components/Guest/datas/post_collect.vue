<template>
  ...
  <!--主体-->
  <div class="usercollect_collect">

    <!-- 上部区域: 单独动态收藏的操作工作区域 -->
    <div class="usercollect_collect_upper">

      <div class="usercollect_collect_control_panel">
        <!--查询按钮-->
        <el-col :span="8">
    ...
    <!-- 下部区域: 具体文件收藏分页查询列表项-->
    <div class="usercollect_collect_lower">
      <!--分页查询的展示框, 提供回车查询绑定, 以及指定查询参数功能-->

      <!--右侧单条折叠操作: 删除-->
      <el-table :data="pageData.records" style="width: 90%">
        <el-table-column prop="title" label="动态名"></el-table-column>
...
const searchFileCollect = () => {
  getUserDataOfPost(pageData.current, pageData.size);
};

// 分页组件页码改变事件
const handleCurrentChange = (newPage) => {
  pageData.current = newPage;
  getUserDataOfPost(newPage, pageData.size);
};
...
// 删除动态收藏 todo 后端做好了再来改
const deleteFileCollect = (row) => {
  http({
    url: http.adornUrl(`Guest/datas/collect/data/post/delete/${row.id}`),
    method: 'delete'
  }).then(() => {
    ElMessage({
      message: '删除成功',
...
.usercollect_collect {
  ...
  .usercollect_collect_upper {
    ...
    .usercollect_collect_control_panel {
      width: 100%;
      display: flex;
    }
  }
